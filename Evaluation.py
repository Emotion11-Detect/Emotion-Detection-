...
def Reg_Verification(Values, array):
    Limit_0_100 = np.array([4, 5, 6, 7, 8, 9, 10])
    # Limit_minus_1_plus_1 = np.array([13])
    if not (np.prod(array) == np.sum(Values[:4])):
        raise Exception("Something went wrong - Please check values")
    # if (not (0 <= Values[Limit_0_100].all() <= 100)) or (
...
def Reg_evaluation(actual, predict):
    Max = preValidation(actual=actual, predict=predict)
    array, [TP, TN, FP, FN] = findConfusionMatrix(actual, predict, Max)
    accuracy = Accuracy(TP, TN, FP, FN)
    sensitivity = Sensitivity(TP, FN)  # (Recall)
    precision = Precision(TP, FP)
    f1score = F1SCORE(TP, FP, FN)
    far = FAR(FP, TN)
    frr = FRR(FN, TP)
    gar = GAR(frr)
    Values = np.asarray([TP, TN, FP, FN, accuracy, sensitivity, precision, f1score, far, frr, gar])
    Reg_Verification(Values, array)
    return Values
