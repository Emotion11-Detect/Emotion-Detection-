...
def ensure_bounds(x: np.ndarray, lb: np.ndarray, ub: np.ndarray):
    """Clamp vector x to box bounds [lb, ub]."""
    return np.minimum(np.maximum(x, lb), ub)


def safe_rand_normal_in_01(mu=0.5, sigma=0.1666):
    """
...
# Improved Orangutan Optimization Algorithm (IOOA) Starting Line No. 58
def Proposed(X, func, lb, ub, max_iter):
    ...
    for t in range(max_iter):
        ...
        for i in range(n_pop):
            ...
            CurrentFit = F[i]
            WorstFit = np.max(F)
            r = CurrentFit / (CurrentFit + (WorstFit * 2))  # r ∈ [0,1], normal (Eq.5)
